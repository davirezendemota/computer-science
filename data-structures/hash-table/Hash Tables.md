

![[Screenshot 2026-03-15 at 12.57.12.png]]
## JUDGMENT
> ==Main rule of the data structure or concept.==
> ==A short sentence that answers: when should I use this?==

---

## LAW OBSERVED
> ==Example of correct usage of the structure.==
> ==Describe a scenario where applying this structure solves the problem efficiently.==

or

## LAW TRANSGRESSED
> ==Example of incorrect usage of the structure.==
> ==Show a scenario where using this structure causes performance or complexity problems.==

---

## KEYS TO POWER
> ==Technical concepts that explain why the structure works==

- **Complexity**: <!-- e.g., access O(1), insertion O(log n), etc -->
- **Internal structure**: <!-- How the data is organized internally -->
- **Main property**: <!-- e.g., ordering, hashing, hierarchy, etc -->
- **Typical behavior**: <!-- Important characteristics of the structure -->

---

## APPLICATION
| ==Practical situations in programming where this structure is used==

- ==Type of problem==
- ==Example of system or feature==
- ==Type of algorithm that commonly uses this structure==

---

## THE INVERSE
> ==When NOT to use this structure.==
> ==Mention other structures that would work better and why.==

---

## IMPLEMENTATION EXAMPLE
==Small code snippet illustrating basic usage==

```pseudo
# simple usage example
structure.operation(value)

```

```mermaid
flowchart LR
	start(key) --> hashFunction
	hashFunction --> buckets
```